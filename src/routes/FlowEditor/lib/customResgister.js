...
export const registerNode = () => {
  ...
  G6.registNode('customLineWithCircle', {
    draw: (cfg, group) => {
  ...
  G6.registNode('customLineWithCircleV', {
    draw: (cfg, group) => {
      group.addShape('circle', {
        attrs: {
  ...
  G6.registEdge('lineNewArrowWithAngel', {
    afterDraw: (cfg, group, keyShape) => {
      const { points } = cfg;
      const start = points[points.length - 2];
      const end = points[points.length - 1];
