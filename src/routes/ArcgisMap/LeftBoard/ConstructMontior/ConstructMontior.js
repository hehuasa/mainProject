    ...
    const content = list.list.length > 0 ? (
      <div className={styles.warp}>
        <div className={styles.header}>
          <div className={styles.name}>{`${list.area.areaName}作业监控`}</div>
                ...
                return (
                  <Panel header={<div className={styles.panelHeader}>{item.jobType}</div>} key={String(item.jobMonitorID)} >
                    <Row type="flex">
                      ...
                      <Col span={10}>申请单位：</Col><Col span={14}>{orgList.filter(value => value.orgID === item.orgID)[0].orgnizationName}</Col>
                      <Col span={10}>装置区域：</Col><Col span={14}>{list.area.areaName}</Col>
