...
@connect(({ emergency }) => ({
  materialList: emergency.materialList,
  areaList: emergency.areaList,
  resourceList: emergency.resourceList,
}))
@Form.create()
export default class SelectMaterial extends PureComponent {
  ...
  render() {
    const { visible, add, handleCancel, form, materialList, name, resourceType, areaList, resourceList } = this.props;
    ...
    return (
      ...
      >
        ...
        <Table
          ...
          dataSource={resourceType === 1 ? resourceList : materialList}
          scroll={{ x: 250 }}
        />
      </Modal>
    );
