...
const mapStateToProps = ({ map }) => {
  const data = [];
  for (const item of map.steamConstantlyValue.data) {
    data.push(item);
  }
  return {
    constantlyValue: { show: map.steamConstantlyValue.show, data },
  };
};
const SteamConstant = ({ constantlyValue }) => {
  return (
...
export default connect(mapStateToProps)(SteamConstant);
