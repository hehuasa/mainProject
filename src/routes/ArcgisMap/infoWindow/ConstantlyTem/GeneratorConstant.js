...
const mapStateToProps = ({ map }) => {
  const data = [];
  for (const item of map.generatorConstantlyValue.data) {
    data.push(item);
  }
  return {
    constantlyValue: { show: map.generatorConstantlyValue.show, data },
  };
};
const GeneratorConstant = ({ constantlyValue }) => {
  return (
...
export default connect(mapStateToProps)(GeneratorConstant);
