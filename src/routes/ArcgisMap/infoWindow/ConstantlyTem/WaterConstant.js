...
const mapStateToProps = ({ map }) => {
  const data = [];
  for (const item of map.waterConstantlyValue.data) {
    data.push(item);
  }
  return {
    constantlyValue: { show: map.waterConstantlyValue.show, data },
  };
};
const WaterConstant = ({ constantlyValue }) => {
  return (
...
export default connect(mapStateToProps)(WaterConstant);
