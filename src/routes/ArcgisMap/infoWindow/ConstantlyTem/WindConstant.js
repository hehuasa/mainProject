...
const mapStateToProps = ({ map }) => {
  const data = [];
  for (const item of map.windConstantlyValue.data) {
    data.push(item);
  }
  return {
    constantlyValue: { show: map.windConstantlyValue.show, data },
  };
};
const WindConstant = ({ constantlyValue }) => {
  return (
...
export default connect(mapStateToProps)(WindConstant);
