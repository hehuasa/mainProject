...
const mapStateToProps = ({ map }) => {
  const data = [];
  for (const item of map.boilerConstantlyValue.data) {
    data.push(item);
  }
  return {
    constantlyValue: { show: map.boilerConstantlyValue.show, data },
  };
};
const BoilerConstant = ({ constantlyValue }) => {
  return (
...
export default connect(mapStateToProps)(BoilerConstant);
