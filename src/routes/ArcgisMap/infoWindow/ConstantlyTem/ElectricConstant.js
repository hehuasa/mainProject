...
const mapStateToProps = ({ map }) => {
  const data = [];
  for (const item of map.electricConstantlyValue.data) {
    data.push(item);
  }
  return {
    constantlyValue: { show: map.electricConstantlyValue.show, data },
  };
};
const ElectricConstant = ({ constantlyValue }) => {
  return (
...
export default connect(mapStateToProps)(ElectricConstant);
