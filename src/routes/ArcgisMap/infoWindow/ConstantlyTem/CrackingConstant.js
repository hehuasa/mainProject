...
const mapStateToProps = ({ map }) => {
  const data = [];
  for (const item of map.crackingConstantlyValue.data) {
    data.push(item);
  }
  return {
    constantlyValue: { show: map.crackingConstantlyValue.show, data },
  };
};
const CrackingConstant = ({ constantlyValue }) => {
  return (
...
export default connect(mapStateToProps)(CrackingConstant);
