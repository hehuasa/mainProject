...
const mapStateToProps = ({ map }) => {
  const data = [];
  for (const item of map.vocConstantlyValue.data) {
    data.push(item);
  }
  return {
    constantlyValue: { show: map.vocConstantlyValue.show, data },
  };
};
const VocConstant = ({ constantlyValue }) => {
  return (
...
export default connect(mapStateToProps)(VocConstant);
