...
const mapStateToProps = ({ map }) => {
  return {
    accessPops: map.accessPops,
  };
};
class AccessPopup extends PureComponent {
  handleClick = () => {
  ...
  render() {
    ...
    const style = {
      left: data.screenPoint ? Number(data.screenPoint.x) : 0,
      top: data.screenPoint ? Number(data.screenPoint.y) - 48 : 0,
    };
    ...
    return (
      <div className={styles.simpleInfo} style={style} >
        ...
        <div>
          <span>{data.in}</span><span>{data.out}</span>
        </div>
        <div
...
export default connect(mapStateToProps)(AccessPopup);
