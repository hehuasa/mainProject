...
//筛选与搜索
const switchData = (data, { endTime, areaName, jobType, startTime }) => {
  const array = [];
  ...
  const array1 = areaName !== '' ? array.filter(value => value.area.areaName.indexOf(areaName) !== -1) : array;
...
class FromComponent extends PureComponent {
  ...
  render() {
    const { startTime, areaName, jobType, endTime } = this.state;
    const { groupingList, form } = this.props;
    const { getFieldDecorator, getFieldsError } = form;
    const newData = switchData(groupingList, { startTime, endTime, areaName, jobType });
    return (
