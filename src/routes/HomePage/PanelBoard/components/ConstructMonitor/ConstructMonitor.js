  ...
  switch (type) {
    ...
    case 0:
      for (const item of list) {
        const { area, count } = item;
        const { areaName, areaId } = area;
        data.push({
          areaName,
          areaId,
          count,
...
export default class ConstructMonitor extends PureComponent {
  ...
  getComponent = (type, list, groupingList) => {
    ...
    const areaData = switchData(groupingList, 0);
    const jobTypeData = switchData(list, 1);
    const noData = '暂无数据';
    switch (type) {
      case 'table':
        return areaData.length > 0 ? <TableList data={groupingList} dispatch={dispatch} list={list} /> : <div style={{ marginTop: 20 }}>{noData}</div>;
      case 'bar':
        return areaData.length > 0 ? <TypeByArea data={areaData} dispatch={dispatch} list={list} /> : <div style={{ marginTop: 20 }}>{noData}</div>;
      case 'bar1':
