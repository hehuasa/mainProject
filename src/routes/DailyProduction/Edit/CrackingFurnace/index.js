...
class CrackingFurnace extends PureComponent {
  constructor(props) {
    super(props);
  ...
  save(from, key) {
    from.validateFields((error, row) => {
      ...
      const newData = this.props.crackingFurnace;
      ...
      if (index > -1) {
        ...
        dispatch({
          type: 'productionDaily/saveCrackingFurnace',
          payload: newData,
        });
        // 保存修改内容
        const { startTimes } = this.props;
        // const { id, 天数, COT, 负荷, stateID } = newData[index];
        const jsonString = { startTimes, ...newData[index] };
        dispatch({
          type: 'productionDaily/updateCrackingFurnace',
  ...
  onTimeChange = (date, dateString) => {
    ...
    this.props.dispatch({
      type: 'productionDaily/getCrackingFurnace',
      payload: { equipmentRunStateID: 1, startTimes },
    });
    this.props.dispatch({
  ...
  render() {
    const { crackingFurnace } = this.props;
    crackingFurnace.map((item, index) => {
      item.key = index;
      return item;
    });
    const components = {
...
export default connect()(CrackingFurnace);
