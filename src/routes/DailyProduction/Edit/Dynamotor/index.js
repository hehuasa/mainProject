...
class Dynamotor extends PureComponent {
  constructor(props) {
    super(props);
  ...
  save(from, key) {
    from.validateFields((error, row) => {
      ...
      const newData = this.props.dynamotor;
      ...
      if (index > -1) {
        ...
        dispatch({
          type: 'productionDaily/saveDynamotor',
          payload: newData,
        });
        // 保存修改内容
        ...
        dispatch({
          type: 'productionDaily/updateDynamotor',
  ...
  onTimeChange = (date, dateString) => {
    ...
    this.props.dispatch({
      type: 'productionDaily/getDynamotor',
      payload: { equipmentRunStateID: 3, startTimes },
    });
    this.props.dispatch({
  ...
  render() {
    const { dynamotor } = this.props;
    dynamotor.map((item, index) => {
      item.key = index;
      return item;
    })
    const components = {
...
export default connect()(Dynamotor);
