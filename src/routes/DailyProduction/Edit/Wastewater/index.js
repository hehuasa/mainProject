...
class Wastewater extends PureComponent {
  constructor(props) {
    super(props);
  ...
  save(from, key) {
    from.validateFields((error, row) => {
      ...
      const newData = this.props.wasteWater;
      ...
      if (index > -1) {
        ...
        dispatch({
          type: 'productionDaily/saveWasteWater',
          payload: newData,
        });
        // 保存修改内容
        ...
        dispatch({
          type: 'productionDaily/updateWasteWater',
  ...
  render() {
    const { wasteWater } = this.props;
    wasteWater.map((item, index) => {
      item.key = index;
      return item;
    });
    const components = {
...
export default connect()(Wastewater);
