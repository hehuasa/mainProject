...
class PanelZoom extends PureComponent{
  ...
  render(){
    ...
    switch (activeKey){
      ...
      case '2':dom= <Table columns={columns}  dataSource={zoomData.data} style={{height:height,padding:'20px 20px 20px,20px'}}   pagination={false} bordered={true} />;break;
      case '3':dom= <Bar   data={zoomData.data}  padding={[45,65,80,100]}/>;break;
      case '4':dom= <Table columns={columns}  dataSource={zoomData.data} style={{height:height,padding:'20px 20px 20px,20px'}}   pagination={false} bordered={true} />;break;
      default : return null
    }
    return (
      (activeKey==="")?<div></div>: <div ref='warp' style={{height:this.state.warpHeight,width:width,transition: 'height 2s'}}><PanelZoomHeader title={zoomData.title} /><div className={styles["panel-zoom-in-content"]}>{dom}</div></div>
    );
}
  // componentDidUpdate(){
